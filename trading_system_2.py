...
class Trader:
    ...
    def squid_ink_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            ...
            # Initialize trader object for squid ink if not exists
            if "squid_ink" not in traderObject:
                traderObject["squid_ink"] = {
                    "price_history": [],
                    "last_fair": mmmid_price,
                    "cycle_period": self.params[Product.SQUID_INK]['cycle_period'],  # Initial guess, will adapt
                    "phase": self.params[Product.SQUID_INK]['phase']
                }
            
            squid_data = traderObject["squid_ink"]
            price_history = squid_data["price_history"]
            
            # Add current mid price to history (keep last 200 data points)
            price_history.append(mmmid_price)
            if len(price_history) > self.params[Product.SQUID_INK]['queue_length']:
                price_history.pop(0)
            ...
            # Detect cycle period using FFT if we have enough data
            if len(price_history) >= self.params[Product.SQUID_INK]['price_history_len']:
                try:
                    ...
                    if dominant_freq != 0:
                        ...
                        # Smooth period update
                        smooth_factor= self.params[Product.SQUID_INK]['smooth_factor']
                        squid_data["cycle_period"] = int(smooth_factor* squid_data["cycle_period"] + 
                                                    (1-smooth_factor) * detected_period)
                except:
                    pass
            
                # Calculate phase in cycle
                if len(price_history) >= 2:
                    # Simple phase detection based on price change direction
                    if price_history[-1] > price_history[-2]:
                        squid_data["phase"] = (squid_data["phase"] + 5) % 360
                    else:
                        squid_data["phase"] = (squid_data["phase"] - 5) % 360
                
                # Calculate sine-based prediction
                cycle_period = max(10, min(200, squid_data["cycle_period"]))  # Bound between 10-200
                sine_factor = math.sin(2 * math.pi * squid_data["phase"] / 360)
                
                # Calculate mean reversion factor
                reversion_factor = (avg_price - mmmid_price) / (std_dev + 1e-6)  # Avoid division by zero
            
                # Combine factors for fair value prediction
                # The sine component captures cyclical patterns
                # The reversion component pulls price back to mean
                sine_component = self.params[Product.SQUID_INK]['sine_factor'] * std_dev * sine_factor  # Scale by volatility
                reversion_component = self.params[Product.SQUID_INK]['reverse_factor'] * std_dev * np.tanh(reversion_factor)  # Smooth reversion
                
                # Calculate final fair value
                fair_value = avg_price + sine_component + reversion_component
                
                # Don't let fair value deviate too much from current market
                fair_value = max(best_bid, min(best_ask, fair_value))
                
                # Store last fair value for reference
